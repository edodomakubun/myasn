...
const Appointment = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        const { error } = await supabase.from('appointment').update(payload).eq('id', editingId);
  ...
  const handleDelete = async (item) => {
    ...
    const { error } = await supabase.from('appointment').delete().eq('id', item.id);
    if (error) setError(error.message);
    ...
    <div>
      ...
      <Table striped bordered hover responsive className="bg-white shadow-sm">
        ...
        <tbody>
          ...
          {dataList.length === 0 && (
            <tr>
              <td colSpan="4" className="text-center py-4 text-muted">Belum ada data SK Pengangkatan.</td>
            </tr>
          )}
        </tbody>
      </Table>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} backdrop="static">
        <Modal.Header closeButton>
          <Modal.Title>{editingId ? 'Edit SK Pengangkatan' : 'Tambah SK Pengangkatan'}</Modal.Title>
        </Modal.Header>
